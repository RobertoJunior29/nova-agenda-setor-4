...
async function baixarNuvem(){
 ...
 gravarLocal();
 pausado=false;
 events.forEach(function(e){vistosLocal[String(e.id)]=true});
 render();
...
async function enviarNuvem(){
 ...
 Object.keys(atuais).forEach(function(k){vistosLocal[k]=true});

 const apagar=[];
 Object.keys(ultimoEstado).forEach(function(k){
  if(k in atuais)return;
  if(!vistosLocal[k])return;
  if(perfil&&perfil.papel!=='admin'&&donos[k]&&donos[k]!==usuario.id)return;
  apagar.push(k);
 });

 if(apagar.length>3){
  console.log('[nuvem] exclusao em massa ignorada por seguranca:',apagar.length,'eventos');
  aviso('A agenda ficou fora de sincronia e '+apagar.length+' exclus\u00f5es foram ignoradas '+
        'por seguran\u00e7a. Recarregue a p\u00e1gina e tente de novo.');
  await baixarNuvem();
 }else{
  for(let i=0;i<apagar.length;i++){
   const d=await sb.from('eventos').delete().eq('id',apagar[i]);
   if(d.error)console.log('[nuvem] nao consegui excluir',apagar[i],d.error.message);
   else{delete ultimoEstado[apagar[i]];delete vistosLocal[apagar[i]]}
  }
 }
...
async function listarPendentes(){
 ...
 _pendentes.forEach(function(row){
  ...
  li.querySelector('.ap-ok').onclick=async function(ev){
   ...
   const u=await sb.from('eventos').update({aprovado:true,recusa:null,recusado_em:null}).eq('id',row.id);
...
function minhasNotificacoes(){
 ...
 events.forEach(function(e){
  ...
  if(donos[id]!==usuario.id)return;
  if(recusas[id])rec.push({e:e,tipo:'recusado',msg:recusas[id]});
  else if(aprovados[id]===false)pen.push({e:e,tipo:'pendente'});
  else apr.push({e:e,tipo:'aprovado'});
 });
 apr.sort(function(a,b){return String(b.e.date||'').localeCompare(String(a.e.date||''))});
 return rec.concat(pen,apr.slice(0,8));
}
...
function atualizarSino(){
 ...
 const recusados=lista.filter(function(x){return x.tipo==='recusado'}).length;
