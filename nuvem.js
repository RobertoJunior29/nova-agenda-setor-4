...
let janelaExclusao=0, _servidor=[], _erroPendentes=null;
...
async function enviarNuvem(){
 ...
 for(let i=0;i<mudados.length;i++){
  ...
  if(eraNovo&&perfil&&perfil.papel!=='admin')carga.aprovado=false;
 ...
 if(meus.length>3){
  ...
  for(let i=0;i<meus.length;i++){
   const k=meus[i];
   const d=await sb.from('eventos').delete().eq('id',k);
   if(d.error)console.log('[nuvem] nao consegui excluir',k,d.error.message);
   else delete ultimoEstado[k];
  }
 }
...
async function listarPendentes(){
 ...
 if(!_pendentes.length){
  box.innerHTML='<p style="font-size:12px;margin:0 0 10px">Nenhum evento pendente.</p>'+
   '<button type="button" class="chip" id="apDump">Ver dados do servidor</button>'+
   '<pre id="apPre" style="display:none"></pre>';
  document.getElementById('apDump').onclick=function(){
   const pre=document.getElementById('apPre');
...
function aplicarClasseAmbito(){
 ...
 for(let i=0;i<cards.length;i++){
  ...
  const ev=events.filter(function(e){return String(e.id)===String(id)})[0];
  if(!ev)continue;
  const congr=(ev.scope===AMBITO_NOVO);
