/* Sincronizacao na nuvem - IEADESGA - v56 */
...
async function enviarNuvem(){
 ...
 const podeExcluir=(Date.now()<janelaExclusao);
 if(faltando.length&&!podeExcluir){
  console.log('[nuvem] ausencias ignoradas (sem acao do usuario):',faltando.length);
  await baixarNuvem();
 }else if(faltando.length>3){
  console.log('[nuvem] exclusao em massa ignorada por seguranca:',faltando.length);
  ...
  for(let i=0;i<faltando.length;i++){
   const k=faltando[i];
   if(perfil&&perfil.papel!=='admin'&&donos[k]&&donos[k]!==usuario.id){
    console.log('[nuvem] exclusao nao permitida para',k);
    continue;
   }
