/* Sincronizacao na nuvem - IEADESGA - v83 */
 ...
 if(meus.length>3){
  ...
  for(let i=0;i<meus.length;i++){
   ...
   if(!d.data||!d.data.length){
    naoApaga[k]=true;
    if(!jaAvisouRecusa['del'+k]){
     jaAvisouRecusa['del'+k]=true;
     aviso(diagnosticoRecusa(k,'excluir'));
    }
    await baixarNuvem();
    return;
   }
...
function marcarVersao(){
 ...
 p.id='versaoAgenda';
 p.textContent='Agenda v83 \u00b7 03.08.2026';
 alvo.appendChild(p);
}
...
async function excluirRecusado(id){
 ...
 if(!r.data||!r.data.length){
  ...
  aviso(diagnosticoRecusa(id,'excluir'));
