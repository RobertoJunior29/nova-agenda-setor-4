...
function marcarPendentesNaTela(){
 ...
 for(let i=0;i<cards.length;i++){
  ...
  if(rec){
   ...
   const meu=(usuario&&donos[String(id)]===usuario.id);
...
function marcarVersao(){
 ...
 p.id='versaoAgenda';
 p.textContent='Agenda v89 \u00b7 03.08.2026';
 alvo.appendChild(p);
}
...
function prepararSequencia(m,id){
 ...
 if(ev&&ev.grupo){
  ...
  box.innerHTML='<label>Sequ\u00eancia</label>'+
   '<div class="seq-info">Dia <b>'+i+'</b> de <b>'+tot+'</b> dias seguidos.'+
   (podeMexer(id)?' <button type="button" class="chip seq-apagar">Excluir a sequ\u00eancia inteira</button>':'')+
   '</div>';
 }else if(!id&&usuario){
  box.innerHTML='<label>Dias seguidos</label>'+
   '<input type="number" id="seqDias" min="1" max="14" value="1" inputmode="numeric">'+
   '<span class="seq-dica">Deixe 1 para um dia s\u00f3. Com 3, o evento \u00e9 criado tamb\u00e9m nos dois dias seguintes.</span>';
 }else{
  return;
 }
 ...
 const inp=document.getElementById('seqDias');
 if(inp){
  inp.addEventListener('change',function(){
   ...
   if(n>14)n=14;
   inp.value=n;
   _seqPendente=(n>1)?{dias:n}:null;
  });
  inp.addEventListener('input',function(){
   let n=parseInt(inp.value,10);
   _seqPendente=(n>1&&n<=14)?{dias:n}:null;
  });
 }
...
function aplicarSequencia(){
 if(!_seqPendente||!_seqPendente.dias||_seqPendente.dias<2)return;
 let idx=-1;
 for(let i=events.length-1;i>=0;i--){
  if(!(String(events[i].id) in ultimoEstado)){idx=i;break}
 }
 ...
 if(base.grupo)return;
 const n=_seqPendente.dias;
 const grupo='g'+Date.now()+'-'+Math.random().toString(36).slice(2,6);
 base.grupo=grupo;base.serieTotal=n;base.serieIndice=1;
 const d0=new Date(String(base.date)+'T12:00');
 if(isNaN(d0)){_seqPendente=null;return}
 for(let k=1;k<n;k++){
  ...
  copia.date=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+
 ...
 gravarLocal();
 if(typeof render==='function')render();
 aviso(n+' eventos criados, um por dia.');
}
...
async function excluirSequencia(grupo){
 ...
 if(!confirm('Excluir os '+ids.length+' eventos desta sequ\u00eancia?'))return;
 ...
 if(!apagados.length){
  aviso('O servidor n\u00e3o autorizou excluir a sequ\u00eancia.');
  return;
 }
 ...
 render();
 atualizarSino();
 aviso(apagados.length+' evento(s) da sequ\u00eancia exclu\u00eddo(s).');
}
...
function prepararDepto(m,id){
 ...
 if(!livre){
  box.querySelectorAll('.dep-op').forEach(function(b){b.disabled=true});
  return;
 }
...
function montarControles(){
 try{ajustarTitulo();ligarTituloMes();montarSeletorAno();montarFiltroMes();
     garantirAmbito();aplicarClasseAmbito();aplicarTagsDepto();montarLegendaDepto()}catch(e){}
}
montarControles();
