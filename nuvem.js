/* Sincronizacao na nuvem - IEADESGA - v25 */
...
function prepararCongregacaoModal(m){
 ...
 function atualizar(){
  ...
  const lista=setor
   ...
   : CONGREGACOES.map(function(c){return c.nome});
  let atual=campo.value;
  const conhecida=CONGREGACOES.some(function(c){return c.nome===atual});
  if(atual&&setor&&conhecida&&lista.indexOf(atual)<0){
   atual='';
   campo.value='';
  }
  if(campo.tagName==='SELECT'){
   ...
   if(!atual&&lista.length===1)campo.value=lista[0];
  }else{
